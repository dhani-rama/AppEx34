...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (deserializeJson(doc, incomingData) == DeserializationError::Ok) {
      bobot = doc["weight"];   // Ambil data bobot dari JSON
      metode = doc["mode"];    // Ambil data metode dari JSON
      int sort = doc["sort"];  // Ambil data jenis terapi dari JSON
      waktu = doc["time"] * 60 * 1000;  // Konversi waktu ke milidetik

      // Set STEP berdasarkan hasil parsing JSON
      if (sort == 1) STEP = 3;  // Kepala
      else if (sort == 2) STEP = 4;  // Pinggang
    }
  }
  ...
  switch (STEP) {
    ...
    case 1:
      ...
      if (Serial.available() > 0) {
        ...
        if (deserializeJson(doc, incomingData) == DeserializationError::Ok) {
              // Ambil data metode dari JSON
          selector = doc["sort"].as<int>();;  // Ambil data jenis terapi dari JSON
    ...
    case 11:
      ...
      if (Serial.available() > 0) {
        ...
        if (deserializeJson(doc, incomingData) == DeserializationError::Ok) {
             metode = doc["mode"].as<int>();    // Ambil data metode dari JSON
             STEP = 2;
        }
      }
      break;
    ...
    case 2:
      ...
      if (Serial.available() > 0) {
        ...
        if (deserializeJson(doc, incomingData) == DeserializationError::Ok) {
             bobot = doc["weight"].as<int>();    // Ambil data metode dari JSON
             STEP = 5;
        }
      }
      break;
    ...
    case 5:
      ...
      if (Serial.available() > 0) {
        ...
        if (deserializeJson(doc, incomingData) == DeserializationError::Ok) {
             waktu = doc["time"].as<int>() * 60 * 1000;    // Ambil data metode dari JSON
